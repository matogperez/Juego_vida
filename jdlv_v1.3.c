...
#include<stdbool.h> //Libreria para que acepte términos booleanos
#include<unistd.h>


int i,j,conteo,x,k,l; //creamos 5 espacios en la memoria para enteros

const int N = 30;  //creamos un espacio para un entero constante que será la base de la matriz
...
int matriz[N][N]; //Declaramos una matriz de nxn. En ste caso le damos explicitamente con número la matriz

int mconteo[N][N]; //Declaramos matriz para almacenar suma de vecinos de matriz original
	...
	while(true){
	//ph++;
	//fors para crear la matriz de conteo
	for(j=0;j<N;j++){
		for(i=0;i<N;i++){
	...
	for(j=0;j<N;j++){
		for(i=0;i<N;i++){
			if((matriz[i][j]==0)&&(mconteo[i][j]==3)){
				
				matriz[i][j] = 1;
			}
				
			else if((matriz[i][j]==1)&&((mconteo[i][j]>3)||(mconteo[i][j]<2))){
